...
try {
    ...
    if (isset($_FILES['compiled_document']) && $_FILES['compiled_document']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    writeLog("File upload status: " . ($file_uploaded ? "File uploaded" : "No file uploaded"));
    ...
    if ($file_uploaded) {
        ...
        writeLog("No file uploaded - skipping file storage");
    }
    ...
    WHERE (role = 'admin' AND department_id IS NULL)  
       OR (role = 'department_admin' AND department_id = ?)
       OR (role = 'superadmin')
");
        $admin_stmt->execute([$department_id]);
        $admins = $admin_stmt->fetchAll(PDO::FETCH_ASSOC);

        writeLog("Found " . count($admins) . " admins for department $department_id");
        ...
        foreach ($admins as $admin) {
            $stmt_notify->execute([
                ...
                "New application submitted for {$service['service_name']}. Tracking: {$tracking_number}",
                'new_application'
            ]);
        }
        ...
        echo json_encode([
            ...
            'original_filename' => $file_uploaded ? $original_filename : null,
            'stored_filename' => $file_uploaded ? $stored_filename : null
        ]);
